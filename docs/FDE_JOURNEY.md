...
- **Retention and deletion, scoped honestly.** The headline used to read "Right to be forgotten" over `forgetShared` alone, which was an overclaim: that function covers the cross-app context bus and nothing else, not the feed, not the narratives, not the tombstones. The real position, since 2026-08-02, is in [RETENTION.md](RETENTION.md): retention windows per data type held as typed data in `lib/retention.ts` and applied by real code (90d `usageCalls`, 30d Ask Pulse turns, 30d cached briefs, 60d introductions), plus a participant deletion path in `scripts/retention/delete-participant.ts` that reports what it cannot reach on every run. `forgetShared` remains the bus half and has to be run alongside it. Still open, and named there rather than glossed: no scheduler for the sweep, no self-service deletion, no window on the feed itself, and Firebase Auth identities are not deleted.
