...
- **Single strong model, not a router.** One model (`claude-opus-4-8`, env-overridable) at low effort for a one-sentence task. Cost is controlled by *caching*, not by model-tiering. Honest: this is not multi-model orchestration.
