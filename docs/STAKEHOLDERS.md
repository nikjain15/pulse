...
## What changed on 2026-08-02: the safety and reliability pass

The reviews below deliberately fixed nothing. This section is the pass that did, and it is written
against the same standard: what closed, what half closed, and what is still a gap wearing a
document instead of a control.

| Item | Status | Where |
|---|---|---|
| **SH10, no dependency scanning in CI** | **Closed.** `npm audit --omit=dev` gates every pull request through `scripts/audit/gate.mjs`. High and critical advisories in shipped dependencies fail the build unless covered by an allowlist entry carrying a package, a reason, a link and an **expiry date that is enforced**. An entry past its date fails the build on its own, whether or not the advisory is still present. RoleOS's version wrote review dates into a free-text reason nobody read; that is the specific failure this design exists to prevent, and `tests/unit/audit-gate.test.ts` is what stops it regressing. Seeded with the three postcss advisories and the sharp advisory, expiring 2026-09-15. A pinned-checksum gitleaks scan covers full history. | `.github/workflows/ci.yml`, `scripts/audit/gate.mjs`, `scripts/audit/allowlist.json` |
| **B2, the high-value suites never ran in CI** | **Closed.** A Firestore emulator job runs the rules and integration suites on every pull request, modelled on Rally's. 203 tests, green locally against the emulator, with the emulator jars cached so the job does not depend on a CDN on the critical path. | `.github/workflows/ci.yml` |
| **SH9, no retention window or deletion path** | **Largely closed.** Windows exist as typed data and are applied by real code, not only documented. A participant deletion path exists and states its own limits on every run. **Open:** no scheduler for the sweep, no self-service button, no window on the feed, Firebase Auth identities untouched. | `lib/retention.ts`, `lib/retention-admin.ts`, `scripts/retention/`, `docs/RETENTION.md` |
| **SH8, no incident response** | **Closed as a runbook, not as an automation.** A rollback specific to Pulse's publish path: kill switch first (unset `ANTHROPIC_API_KEY`, which lands on the product's own designed facts-only state), then redact published sentences while keeping the evidence, then revert the prompt or model, then turn narration back on against a named health threshold, then turn the incident into fixture rows. Each step has a duration and a signal. | `docs/RUNBOOK.md`, `scripts/incident/redact-narratives.ts` |
| **SH3, no alert threshold** | **Half closed, and the half that is missing is named in the payload.** `lib/health.ts` defines what "broken" means as numbers: degradation rate over 20% warns and over 50% alerts, above a 20-attempt minimum sample; guard-rejection rate over 10% alerts separately; spend over $0.75/day alerts against an $11 pilot budget. `/api/usage` now returns the verdict. **Nothing notifies anybody.** No pager, no cron, no webhook. The response body says so in a `notice` field and a unit test asserts it keeps saying so. | `lib/health.ts`, `app/api/usage/route.ts` |
| **E1, the backwards CI row cap** | **Closed.** The assertion was a ceiling of 60 rows on the safety fixture, so CI failed if the dataset that most needed growing grew. It is now a floor of 70 with a 25-row minimum per class. The fixture grew 49 to 76 rows and the residual list is now explicit. | `tests/unit/eval-metrics.test.ts`, `evals/guard-fixture.json` |

**Not touched, and still open from the reviews:** S1 (the guard runs on the generation path, not the
write path), S2 (caller-supplied peer list), S5 (rate-limit ceiling), D1 to D6 (every design and
accessibility finding), E2 (the circular groundedness eval), E3 (a judge that cannot fail), E5
(metric weighting). None of them got easier; they simply were not this pass.
...
| S3 | P1 | The opt-out tombstone list is world-readable. `firestore.rules:332` is `allow read: if true`, so the enumerable list of everyone who asked to be hidden is public, which is precisely the privacy-sensitive fact the feature exists to protect. Already disclosed in README.md and in the rules comment; restated here because disclosure is not mitigation. | **Still open, and now documented rather than only disclosed** (2026-08-02). `docs/RETENTION.md` records why it cannot be closed today (the pre-index reads through the client SDK with no credential, so tightening the rule would break the filter and show people who opted out) and why hashing the handles was considered and rejected (the candidate set is a public 65-login repository, so a hash is enumerable in 65 operations). What did change: `fetchOptOuts` had a second, undocumented hole beside this one, reading a single 300-document page and dropping `nextPageToken`, so the 301st person to opt out would have silently reappeared on the landing page. It now paginates and fails closed, covered by `tests/unit/opt-out.test.ts`. The original finding stands. |
| S4 | P1 | There is no written retention window and no deletion path. A member can delete individual events (`firestore.rules:265`), but nothing purges a person's history, `optOuts` is create-only and permanent (`firestore.rules:344`), and `usageCalls` rows accumulate forever (`lib/usage-admin.ts:56`). `docs/FDE_JOURNEY.md:23` headlines `forgetShared` as "Right to be forgotten", but that function covers only the shared-context bus, not the feed, the narratives, or the tombstones. The framing overreaches the mechanism. | **Largely closed** (2026-08-02, SH9). `lib/retention.ts` is the policy as typed data, `docs/RETENTION.md` is its prose. Windows: 90d `usageCalls`, 30d `askThreads` turns, 30d `briefs`, 60d `introductions`, enforced by real code, not documentation. `usageCalls` and `askThreads` prune on their write path, which needs no credential and therefore runs in every deployment; `briefs` and `introductions` need the sweep in `scripts/retention/sweep.ts`. `scripts/retention/delete-participant.ts` is the deletion path that did not exist, and it prints what deletion cannot reach on every run, including dry runs. **Still open:** no scheduler for the sweep, no self-service deletion button, no window at all on `pulse`/`tasks`/`recipes`/`comments`, and Firebase Auth identities are not deleted. |
...
**Since closed, in part** (2026-08-02, SH10): `npm audit` now gates CI through
`scripts/audit/gate.mjs`, with an allowlist whose expiry dates are enforced rather than decorative;
a pinned-checksum gitleaks secret scan runs over full history; and the workflow now declares
`permissions: contents: read`. Still not looked at: the vendored `conduit/` code's provenance, and
the lockfile itself.
...
| E4 | P1 | The homoglyph blind spot is the most likely way the headline number is wrong in production. `foldForMention` (`lib/sense.ts:503-509`) folds combining marks and zero-width characters but not cross-script confusables, and the fixture does not exercise it. It is disclosed twice (`lib/sense.ts:501`, `docs/EVALS.md:62`) and it is still the cheapest evasion available: one character in a commit message. Until a confusables fold lands and rows are added, the fixture measures the guard against the evasions its own author already thought of, which is the definition of a contaminated test set. | **The blind spot is now in the fixture; the fold is not fixed** (2026-08-02). `guard-fixture.json` carries a `knownResiduals` list of four evasions that genuinely defeat the guard today: the Cyrillic homoglyph, a soft hyphen spliced into a name, a name spelled with separating spaces, and a Turkish dotless `ı`. They are deliberately excluded from the scored rows, because scoring them would report a recall under the 1.0 safety floor and the tempting fix would be to relabel them as allowed. `tests/unit/eval-metrics.test.ts` asserts each one still evades, so the day `foldForMention` is widened, CI goes red and says to promote it. The contaminated-test-set criticism stands: the fixture still measures the guard against evasions its own author thought of. |
...
**Since changed** (2026-08-02, B2): those suites are no longer read-only in this sense either. CI
runs the rules and integration suites in a Firestore emulator job on every pull request, so 203 of
them now gate a merge instead of running when Nik remembers `npm run gate`. Their quality is still
unreviewed; what changed is that they run.
